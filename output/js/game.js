...
export class GameScene {
    ...
    redraw(c) {
        ...
        // Underlying part
        if (this.monsterActive) {
            c.drawBitmapRegion(c.getBitmap("monster"), 0, 0, 540, 288, 0, this.monsterPos);
        }
        for (let e of this.enemies) {
            e.draw(c);
        }
        ...
        // Overlaying part
        if (this.monsterActive) {
            c.drawBitmapRegion(c.getBitmap("monster"), 0, 288, 540, 288, 0, this.monsterPos);
        ...
        if (this.guideTimer > 0) {
            ...
            c.drawBitmapRegion(bmpGuide, 0, 0, 128, 320, -bmpGuide.width / 2 + bmpGuide.width / 2 * t, c.height - bmpGuide.height);
            c.drawBitmapRegion(bmpGuide, 128, 0, 128, 320, c.width - bmpGuide.width / 2 * t, c.height - bmpGuide.height);
        }
        c.moveTo();
        c.setFillColor(0, 0, 0, 0.33);
        c.fillRect(0, 0, c.width, 32);
        c.drawText(c.getBitmap("font"), "DEPTH: " + this.genDepthString() + "%", c.width / 2, 0, -26, -2, true, 0.5, 0.5);
        if (this.paused) {
            c.setFillColor(0, 0, 0, 0.67);
            c.fillRect(0, 0, c.width, c.height);
            c.drawText(c.getBitmap("font"), "GAME PAUSED", c.width / 2, c.height / 2 - 32, -28, 0, true, 1, 1, this.textWave, 8, Math.PI * 2 / 5);
        }
    }
    dispose() {
        return null;
...
GameScene.GUIDE_DISAPPEAR_TIME = 30;
